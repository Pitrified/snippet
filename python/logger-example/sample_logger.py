"""MAKEDOC: sample_logger module level docstring."""
import argparse
import logging
...
def setup_env() -> argparse.Namespace:
    ...
    file_fmt_type = "lanm"
    file_log_level = "WARN"
    file_log_path = Path("log_sample_logger.log")
    file_log_mode = "a"
    setup_logger(
...
def run_sample_logger(args: argparse.Namespace) -> None:
    r"""MAKEDOC: What is sample_logger doing?

    Args:
        args: The cmdline arguments.
    """
    logg = logging.getLogger(f"c.{__name__}.run_sample_logger")
    logg.setLevel("DEBUG")
    logg.debug("Starting run_sample_logger")


if __name__ == "__main__":
    args = setup_env()
    run_sample_logger(args)
